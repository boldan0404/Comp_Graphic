...
class Trimesh : public SceneObject {
  ...
  mutable KdTree<TrimeshFace*>* faceKdTree;

public:
  Trimesh(Scene *scene, Material *mat, MatrixTransform transform)
      : SceneObject(scene, mat), displayListWithMaterials(0),faceKdTree(nullptr),
  ...
  void buildKdTreeForFaces() const;
  void clearFaceKdTree() const { 
    if(faceKdTree) { delete faceKdTree; faceKdTree = nullptr; }
  }

  glm::dvec3 getVertex(int index) const { return vertices[index]; }
